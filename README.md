...
| `permodel` | Per-model regression router = R2-Router's per-model router ([UCF-ML-Research/R2-Router](https://github.com/UCF-ML-Research/R2-Router), `r2_router/router.py`) specialized to 2 models, a single budget tier, and cost `c_weak=0, c_strong=1`. Each model gets an independent **Ridge** `P(pass\|emb)` predictor; R2's `risk=(1−λ)·quality−λ·cost` reduces (constant per-pair cost) to ranking by `P_strong−P_weak`, so score `=(P_strong−P_weak+1)/2` and the threshold sweep **is** the λ sweep |
| `permodel_cluster` | **Our own extension, not part of R2-Router.** `permodel` whose regression input is augmented with UniRoute per-cluster signals (ψ_weak, ψ_strong), reused verbatim from the trained UniRoute (honest K, Ψ=train) checkpoint via `--uniroute-checkpoint`. (R2-Router's actual UniRoute fusion, `unirouter/uni_r2.py`, instead uses cluster-averaged quality `Φ·Ψ` — that corresponds to `uniroute` (soft), **not** to feeding ψ as regression features.) |
