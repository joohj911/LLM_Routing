...
| `uniroute` | UniRoute K-Means cluster-based router ([arXiv:2502.08773](https://arxiv.org/abs/2502.08773)); **hard** nearest-cluster assignment. Variants `uniroute_train` (final Ψ on full train) and `uniroute_legacy` (legacy circular K selection) |
| `uni_r2` | R2-Router's UniRoute fusion (`unirouter/uni_r2.py`): quality = `Φ(x)·Ψ(h)` with **soft** (softmax) cluster membership Φ and per-cluster quality Ψ, under the R2 `risk=(1−λ)q−λc` objective. Reduced to 2 models / single budget / cost 0/1, this is exactly `uniroute` with honest K, Ψ=train, and **soft** assignment — placed next to hard `uniroute_train` to reproduce the repo's "UniRouter (hard) vs Uni-R2 (soft)" comparison |
| `r2_router` | R2-Router's per-model router ([UCF-ML-Research/R2-Router](https://github.com/UCF-ML-Research/R2-Router), `r2_router/router.py`) specialized to 2 models, a single budget tier, and cost `c_weak=0, c_strong=1`. Each model gets an independent **Ridge** `P(pass\|emb)` predictor; R2's `risk=(1−λ)·quality−λ·cost` reduces (constant per-pair cost) to ranking by `P_strong−P_weak`, so score `=(P_strong−P_weak+1)/2` and the threshold sweep **is** the λ sweep |
| `cscr` | CSCR — Cost-Aware Contrastive Routing ([arXiv:2508.12491](https://arxiv.org/abs/2508.12491), [rezashkv/cscr](https://github.com/rezashkv/cscr)). **Expert descriptors are computed from the real models' output** (logit footprint: top-256 vocab softmax over 10 greedy tokens on probe prompts, L2-normalized). A frozen-e5 + 2-layer MLP `g_θ` is trained with the paper's cost-spectrum InfoNCE (Eq. 8; K cost bands, band temperature Eq. 7, negative cost penalty λ) to project prompts into descriptor space; routing is cosine nearest-descriptor (FAISS `flat_ip`). Opt-in via `run_experiments.sh --with-cscr` (needs GPU probe inference). |

`cscr` is a self-contained router (not an embedding swap): enable it with `run_experiments.sh --with-cscr`, which computes real-model logit descriptors then trains and evaluates the contrastive router alongside the others.
